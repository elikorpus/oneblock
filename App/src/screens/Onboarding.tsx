...
import { formatISODate } from '../lib/dateTimeFormat';
...
export function OnboardingScreen({ navigation }: Props) {
  ...
  const aboutOk = about.firstName.trim().length > 0 && !!about.birthday;
  ...
  const finish = async () => {
    ...
    try {
      await completeSignup({
        ...
        interests: picked,
        family,
        birthday: about.birthday ? formatISODate(about.birthday) : '',
        isPrivate: about.isPrivate,
      });
    } catch (e) {
        ...
        {step === 2 && (
          <ScrollView contentContainerStyle={styles.stepContent} keyboardShouldPersistTaps="handled">
            ...
            <DateTimeField
              ...
              onChange={(d) => setAbout({ ...about, birthday: d })}
              placeholder="Choose your birthday"
            />
            <View style={styles.privacyRow}>
              <View style={{ flex: 1, paddingRight: 12 }}>
                <Text style={styles.privacyLabel}>Keep my profile private</Text>
                <Text style={styles.privacyNote}>
                  Neighbors will only see your name and shared interests — your address, contact info, bio, and
                  household stay hidden. You can change this anytime from your profile.
                </Text>
              </View>
              <Switch
                value={about.isPrivate}
                onValueChange={(v) => setAbout({ ...about, isPrivate: v })}
